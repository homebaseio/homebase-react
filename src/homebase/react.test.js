  ...
  return (
    <>
      <div data-testid="jsonAnyQueryWrap">
        {jsonAnyQuery.map(item => <div key={item.get('id')} data-testid="jsonAnyQuery">{item.get('name')}</div>)}
      </div>
      <div data-testid="jsonAttrQueryWrap">
        {jsonAttrQuery.map(item => <div key={item.get('id')} data-testid="jsonAttrQuery">{item.get('name')}</div>)}
      </div>
      <div data-testid="jsonAttrAndQueryWrap">
        {jsonAttrAndQuery.map(item => <div key={item.get('id')} data-testid="jsonAttrAndQuery">{item.get('name')}</div>)}
      </div>
      <div data-testid="datalogQueryWrap">
        {datalogQuery.map(item => <div key={item.get('id')} data-testid="datalogQuery">{item.get('name')}</div>)}
      </div>
    </>
  )
}
...
test('useQuery', async () => {
  ...
  expect(screen.getAllByTestId('jsonAttrAndQuery')).toHaveLength(1)
  expect(screen.getByTestId('jsonAttrAndQueryWrap')).toHaveTextContent('id lookup')
  
  expect(screen.getAllByTestId('datalogQuery')).toHaveLength(1)
...
const GetComp = () => {
  ...
  return (
    <>
      ...
      <div data-testid="order.items.1.ref.parent.name">{order.get('items', 1, 'ref', 'parent', 'name')}</div>
      <div data-testid="order.items.1.ref.parent._parent.name">{order.get('items', 1, 'ref', 'parent', '_parent', 'name')}</div>
      ...
      <div>
        {order.get('items', 'ref').map(item => (
          <div key={item.get('id')} data-testid="order.items">{item.get('name')}</div>
        ))}
      </div>
      <div>
        {order.get('items', 'ref', 'name').map(name => (
          <div key={name} data-testid="order.items.ref.name">{name}</div>
        ))}
      </div>
    </>
  )
...
test('entity.get()', async () => {
  ...
  expect(screen.getByTestId('order.items.1.ref.parent._parent.name')).toHaveTextContent('identity lookup')
...
const TransactComp = () => {
  ...
  return (
    <>
      ...
      <button onClick={() => transact([{ item: { name: 'new item' } }])}>
        create|newItem
      </button>
...
const ClientComp = () => {
  ...
  return (
    <>
      ...
      <button onClick={() => client.transactSilently([{ order: { id: order.get('id'), name: 'order1' } }])}>
        update|order.name
      </button>
      <div data-testid="order.name">{order.get('name')}</div>
      <button onClick={() => client.dbFromString(initialDBString)}>
        client.dbFromString()
      </button>
    </>
  )
...
test('useClient', async () => {
  render(<ClientApp />)
  expect(screen.getByTestId('client.dbToString()')).toHaveTextContent(initialDBString)
  expect(screen.getByTestId('client.dbToDatoms()')).toHaveTextContent(JSON.stringify(initialDBDatoms))
  fireEvent.click(screen.getByText('update|order.name'))
  expect(screen.getByTestId('order.name')).toHaveTextContent('order1')
